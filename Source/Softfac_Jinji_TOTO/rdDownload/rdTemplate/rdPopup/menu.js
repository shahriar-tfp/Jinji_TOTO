 ...
 */
 ...
 var rdWrotePopupMenus=false;  //Only run rdWritePopupMenus once.  Otherwise, could run more often with SubReprts.
 
 function rdWritePopupMenus(menuWidth) {
	if (rdWrotePopupMenus==true){
		return;
	}
	rdWrotePopupMenus=true;
	var aMenus = document.getElementsByTagName("rdPopupMenu");
	for (var i = 0; i < aMenus.length; i++) {
		var popupID = "rdPopupMenu-" + aMenus[i].getAttribute("id")
		window.popupMenu = new Menu(popupID);
		
		if (document.all) {  //For IE 
			var menuItem = aMenus[i].nextSibling
			while (true) {
				if (menuItem.tagName == "RDPOPUPMENUITEM") {
					//This is an item of the popup menu.
					var label = menuItem.getAttribute("LABEL")
					var action = menuItem.getAttribute("ACTION")
					if (action.indexOf("ShowElement(") != -1) {
						if (menuItem.id.indexOf("_Row") != -1) {
							var row = menuItem.id.substr(menuItem.id.indexOf("_Row"))
							action = action.replace("this.id","'" + row + "'")
						}
					}
					popupMenu.addMenuItem(label,action)
				}
				if (menuItem.tagName == "/RDPOPUPMENUITEMS") {
					//Reached the end of this menu.
					break;
				}
			...
			var aMenuItems = aMenus[i].childNodes[0].childNodes
			for (var k = 0; k < aMenuItems.length; k++) {
				var menuItem = aMenuItems[k]
				if (menuItem.tagName == "RDPOPUPMENUITEM") {
					//This is an item of the popup menu.
					var label = menuItem.getAttribute("LABEL")
					var action = menuItem.getAttribute("ACTION")
	...
	if (aMenus.length != 0) {
		writeMenus(null,menuWidth)
	}
}

function rdShowMenu(menuID) {
	showMenu(window.menus["rdPopupMenu-" + menuID])
}
...
function writeMenus(container, menuWidth) {
    ...
    if (document.layers) {
        ...
        for (var x=0; x<container.menus.length; x++) {
            ...
            proto.menuItemWidth = proto.menuItemWidth || menuWidth || 200;

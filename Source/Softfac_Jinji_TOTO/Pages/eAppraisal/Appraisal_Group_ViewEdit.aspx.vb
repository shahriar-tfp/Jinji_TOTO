...
Partial Class Pages_EAPPRAISAL_APPRAISAL_GROUP_ViewEdit
...
#Region "Panel Edit"
    ...
    Protected Sub ddlOPTION_BYLIST_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlOPTION_BYLIST.SelectedIndexChanged
        ...
        If ddlOPTION_BYLIST.SelectedValue = "1" Or ddlOPTION_BYLIST.SelectedValue = "2" Then
    ...
    Protected Sub imgbtnGROUP_CODE2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnGROUP_CODE2.Click
        ...
        If Validatefield() Then
            ssql2 = "select dbo.fn_GetEmpIDUsingCodeName(N'" & txtEMPLOYEE_PROFILE_ID.Text & "')"
            myDS1 = mySQL.ExecuteSQL(ssql2, Session("Company").ToString, Session("EmpID").ToString)
            If myDS1.Tables(0).Rows.Count > 0 Then
                ssql = "Exec sp_sa_getListBoxValue 'Appraisal','" & Session("Company") & "','" & ddlOPTION_BYLIST.SelectedValue & "','" & myDS1.Tables(0).Rows(0).Item(0).ToString & "','" & ddlOPTION_TYPE.SelectedValue & "','" & ddlAPPRAISAL_MONTH.SelectedValue & "','',''"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                If myDS.Tables(0).Rows.Count > 0 Then
        ...
        If Validatefield() Then
            If lstleft.Visible = True Then
                If lstright.Visible = True And lstright.Items.Count > 0 Then
                    ssql1 = "Select dbo.fn_GetEmpIDUsingCodeName(N'" & txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_GetEmpIDUsingCodeName(N'" & txtEMPLOYEE_PROFILE_ID_VALUE.Text & "')"
                    ...
                    If myDS1.Tables(0).Rows.Count > 0 Then
                        ...
                        For k = 0 To itemCount - 1
                            ...
                            ssql2 = Nothing
                        Next
                        ssql = "Exec sp_is_insupddelAppraisalGroup N'UPD',N'" & Session("Company").ToString & "',N'" & myDS1.Tables(0).Rows(0).Item(0).ToString & "',N'" & ddlOPTION_BYLIST.SelectedValue & "','" & ddlOPTION_TYPE.SelectedValue & "','" & myDS1.Tables(0).Rows(0).Item(1).ToString & "'," & spid & ",'" & Session("EmpID").ToString & "','" & ddlAPPRAISAL_MONTH.SelectedValue & "'"

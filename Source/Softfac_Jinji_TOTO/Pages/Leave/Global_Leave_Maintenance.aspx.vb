...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        mySetting.GetDropdownlistValue("GLOBAL_APPLY_LEAVE_VW", "OPTION_TYPE", ddlOPTION_TYPE)
...
#Region "Panel Edit"
    ...
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        If Validate1() = True Then
            ssql1 = "select dbo.fn_ReturnCompanyProfileCode('" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_GetComOCPIDBYCODENAME('" & txtOCP_ID_LEAVE.Text & "','LEAVE',dbo.fn_ReturnCompanyProfileCode('" & txtCOMPANY_PROFILE_CODE.Text & "'))"
            myDS1 = mySQL.ExecuteSQL(ssql1)

            If myDS1.Tables.Count > 0 Then
                If myDS1.Tables(0).Rows.Count > 0 Then
                    Do While vCount < lstright.Items.Count
                        ssql = "Exec SP_ls_InsUpdLeaveMaintenance """ & myDS1.Tables(0).Rows(0).Item(0).ToString & """,""" & myDS1.Tables(0).Rows(0).Item(1).ToString & """,""" & Trim(Left(lstright.Items(vCount).Value.Trim, 10)) & _
                        """,""" & mySetting.UnDisplayDateTime(txtEFFECTIVE_DATE.Text, Session("Company").ToString, Session("Module").ToString) & """,""" & mySetting.UnDisplayDateTime(txtEXPIRY_DATE.Text, Session("Company").ToString, Session("Module").ToString) & _
                        """," & txtDAYS.Text & ",""" & txtREASON.Text & """,""" & Session("EmpID").ToString & """,'ADD'"
                        myDS = mySQL.ExecuteSQL(ssql)

                        'If myDS.Tables(0).Rows(0).Item(0).ToString = "ERROR" Then
                    ...
                    Loop
                    If ErrCnt > 0 Then
                        lblresult2.Text = "Leave Credit successfully..."
                    Else
                        lblresult2.Text = "Leave Credit successfully..."
                    End If
                End If
...
#Region "Sub & Function"
    ...
    Private Function Validate1() As Boolean
        ...
        If txtDAYS.Text = "" Then
            lblresult2.Text = lblDAYS.Text & " field is required..."
            Exit Function
        End If
        If txtEFFECTIVE_DATE.Text = "" Or mySetting.CheckIsDateNull(txtEFFECTIVE_DATE.Text) Then
            lblresult2.Text = lblEFFECTIVE_DATE.Text & " field is required..."
            Exit Function
        End If
        If txtEXPIRY_DATE.Text = "" Or mySetting.CheckIsDateNull(txtEXPIRY_DATE.Text) Then
            lblresult2.Text = lblEXPIRY_DATE.Text & " field is required..."
            Exit Function
        End If
        If mySQL.fn_DateDiff("day", mySetting.UnDisplayDateTime(txtEFFECTIVE_DATE.Text, Session("Company").ToString, Session("Module").ToString), mySetting.UnDisplayDateTime(txtEXPIRY_DATE.Text, Session("Company").ToString, Session("Module").ToString)) < 0 Then
            lblresult2.Text = lblEXPIRY_DATE.Text & " field is required..."
            Exit Function
        End If
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "EFFECTIVE_DATE" Then
                    If lblEFFECTIVE_DATE.Visible = True And txtEFFECTIVE_DATE.Visible = True Then
                        autonum += 1
                        imgEFFECTIVE_DATE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgEFFECTIVE_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgEFFECTIVE_DATE.Style.Add("position", "absolute")
                        autonum += 1
                        lblEFFECTIVE_DATE.CssClass = "wordstyle12"
                        lblEFFECTIVE_DATE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblEFFECTIVE_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ...
                        txtEFFECTIVE_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ...
                        imgBtnEFFECTIVE_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgBtnEFFECTIVE_DATE.Style.Add("position", "absolute")
                    End If
                End If

                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "EXPIRY_DATE" Then
                    If lblEXPIRY_DATE.Visible = True And txtEXPIRY_DATE.Visible = True Then
                        autonum += 1
                        imgEXPIRY_DATE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgEXPIRY_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgEXPIRY_DATE.Style.Add("position", "absolute")
                        autonum += 1
                        lblEXPIRY_DATE.CssClass = "wordstyle12"
                        lblEXPIRY_DATE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblEXPIRY_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblEXPIRY_DATE.Style.Add("position", "absolute")
                        autonum += 1
                        txtEXPIRY_DATE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        txtEXPIRY_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        txtEXPIRY_DATE.Style.Add("position", "absolute")
                        txtEXPIRY_DATE.Width = txtWidth
                        autonum += 1
                        imgBtnEXPIRY_DATE.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgBtnEXPIRY_DATE.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgBtnEXPIRY_DATE.Style.Add("position", "absolute")
                    End If
                End If

                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "DAYS" Then
                    If lblEXPIRY_DATE.Visible = True And txtDAYS.Visible = True Then
                        autonum += 1
                        imgDAYS.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgDAYS.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgDAYS.Style.Add("position", "absolute")
                        autonum += 1
                        lblDAYS.CssClass = "wordstyle12"
                        lblDAYS.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblDAYS.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblDAYS.Style.Add("position", "absolute")
                        autonum += 1
                        txtDAYS.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        txtDAYS.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        txtDAYS.Style.Add("position", "absolute")
                        txtDAYS.Width = txtWidth
                        autonum += 1
                        imgBtnDAYS.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgBtnDAYS.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgBtnDAYS.Style.Add("position", "absolute")
                    End If
                End If

...
Partial Class Pages_Payroll_Process_Batch_Payroll
    ...
    Private Sub ShowBatchState()
        ...
        myDS = mySQL.ExecuteSQL("select convert(nvarchar,querydate) as Date,state as State,Userid,Message,ProcessTime,Active, Resign, NewJoin, Skip, ProcessYear,ProcessMonth from dbo.Payroll_Batch_Process where company_profile_code = '" & Session("Company").ToString & "' order by querydate desc", Session("Company").ToString, Session("EmpID").ToString)
    ...
    Private Sub PagePreload()
        ...
        'get Page Title
        ssql = "exec sp_sa_GetPageTitle 'PAYROLL_BATCH_PROCESS_VW'"
        myDS = New DataSet
    ...
    Protected Sub imgbtnPROCESS_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnPROCESS.Click
        ...
        If ValidateProcess() = True Then
            ...
            Session("SPID") = CInt(SPID) + CInt(8100 * random.NextDouble())

            aaa = Session("SPID").ToString
                ...
                For i = 0 To myGridView.Rows.Count - 1
                    ...
                    If chkBox.Checked = True Then
                        ...
                        Else
                            query.Append(" ; ")
                        End If
                        ssql = "Insert Into Employee_Payroll_Process_Temp Values(N'" & Session("Company").ToString & "'," & _
                                                ...
                                                ddlMONTH.SelectedValue & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & Session("SPID").ToString & "')"
            ...
            Else
                ...
                ssql = "Exec sp_pr_InsertEmployeeForPayrollProcess N'" & Session("Company").ToString & "',N'" & Session("Module").ToString & "',N'" & Session("EmpID").ToString & _
                    "',N'" & txtYEAR.Text.ToString & "',N'" & ddlMONTH.SelectedValue & "',N'" & ddlPAY_CYCLE.SelectedValue.ToString & "',N'" & ddlFILTER_BY.SelectedValue.ToString & _
                    "',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Trim & "',N'" & Session("SPID").ToString & "'"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                ssql = Nothing
            ...
            query.Append(" ^ select user_profile_code='PROBATCHMGR' into #audit  Exec sp_pr_calPayrollProcessing N'" & Session("Company").ToString & "',N'" & _
                            ddlPAY_CYCLE.SelectedValue & "',N'" & Trim(ddlMONTH.SelectedValue) & "',N'" & _
                            Trim(txtYEAR.Text.ToString) & "',N'" & ddlFILTER_BY.SelectedValue & "',N'" & Session("SPID").ToString & "'")
            num += 1

            Dim apppath, filepath, currentdate As String
            Dim filename2 As String
            apppath = Request.PhysicalApplicationPath
            currentdate = Date.Today.Year.ToString & Date.Today.Month.ToString & Date.Today.Day.ToString & Date.Now.Hour.ToString & Date.Now.Minute.ToString & Date.Now.Second.ToString

            filepath = apppath & "Pages\Batch_Process\" & currentdate & num & Session("SPID").ToString & ".txt"
            Dim w As StreamWriter

            w = File.CreateText(filepath)
            w.WriteLine(query.ToString)
            w.Flush()
            w.Close()
            ...
            Dim result As String = mySQL.AddBatchQuery(query, Session("EmpID").ToString(), num, batchdate, Session("Company").ToString, txtYEAR.Text, ddlMONTH.SelectedValue, ddlPAY_CYCLE.SelectedValue)
            ...
            End If
            ssql = "select num from Payroll_Batch_Process where userid = '" & Session("EmpID").ToString & "' and count = " & num & " and querydate = '" & batchdate & "' and company_Profile_code = '" & _
            Session("Company").ToString & "' and processyear = '" & txtYEAR.Text & "' and Processmonth = '" & ddlMONTH.SelectedValue & "'"
            myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            ...
            If myDS.Tables.Count > 0 Then
                If myDS.Tables(0).Rows.Count > 0 Then
                    KeyNum = myDS.Tables(0).Rows(0).Item(0).ToString()
                End If
            End If
            ssql = Nothing

            ssql2 = "insert into Payroll_Batch_Process2 select " & KeyNum & " ,'" & filename2 & "'"

...
#Region "Public Declaration"
    ...
    Public fromdate As String, todate As String
...
#Region "Panel Edit"
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        End If
        lblresult2.Text = ""
        If ValidateCHK("ADD", "", "") = True Then
            ssql = "Exec sp_pr_insupddelchkEmpBenefit '" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "','" & txtOCP_ID_SALARY.Text.Replace("'", "''") & "','" & txtDATE_FROM.Text & "','" & txtDATE_TO.Text & _
            "','','','" & Session("Company") & "','Modify','ADD','" & ddlOPTION_PAY_CYCLE.SelectedValue & "','" & txtVALUE.Text & "','" & ddlOPTION_PAY_TYPE.SelectedValue & "','" & ddlOPTION_CALCULATE_ON.SelectedValue & "'"
    ...
    Protected Sub imgBtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdate.Click
        If Session.IsNewSession Then
            Response.Redirect("../Global/SessionTimeOut.aspx")
        End If
        fromdate = txtADate_From.Text
        todate = txtADate_To.Text
        If ValidateCHK("UPD", fromdate, todate) Then
            ssql = "Exec sp_pr_insupddelchkEmpBenefit '" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "','" & txtOCP_ID_SALARY.Text.Replace("'", "''") & "','" & txtDATE_FROM.Text & "','" & txtDATE_TO.Text & _
            "','" & fromdate & "','" & todate & "','" & Session("Company") & "','Modify','UPD','" & ddlOPTION_PAY_CYCLE.SelectedValue & "','" & txtVALUE.Text & "','" & ddlOPTION_PAY_TYPE.SelectedValue & "','" & ddlOPTION_CALCULATE_ON.SelectedValue & "'"
            mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            pnlEdit.Visible = False
            pnlMain.Visible = True
            BindGrid()
                                ...
                                Select Case myDS.Tables(0).Rows(j).Item(1).ToString.Trim
                                    ...
                                    Case Else
                                        ...
                                        Else
                                            ...
                                            If myDS.Tables(0).Rows(j).Item(0).ToString.Trim = "DATE_FROM" Then
                                                txtADate_From.Text = Replace(Replace(Server.HtmlDecode(gvHistory.Rows(i).Cells(intColumn).Text.ToString.Trim), "&nbsp;", "&"), "&amp;", "&")
                                            End If
                                            If myDS.Tables(0).Rows(j).Item(0).ToString.Trim = "DATE_TO" Then
                                                txtADate_To.Text = Replace(Replace(Server.HtmlDecode(gvHistory.Rows(i).Cells(intColumn).Text.ToString.Trim), "&nbsp;", "&"), "&amp;", "&")
                                            End If
                                        End If
                                        myTXT = Nothing
                                End Select
        ...
        If ActionValidateDelete() Then
            For i = 0 To myGridView.Rows.Count - 1
                ...
                If chkDelete.Checked Then
                    ssql = "Exec sp_pr_insupddelchkEmpBenefit '" & myGridView.Rows(i).Cells(1).Text.Replace("'", "''") & "','" & myGridView.Rows(i).Cells(2).Text.Replace("'", "''") & "','" & myGridView.Rows(i).Cells(3).Text.Replace("'", "''") & "','" & myGridView.Rows(i).Cells(4).Text.Replace("'", "''") & _
                    "','','','" & Session("Company") & "','Modify','DEL','" & gvHistory.Rows(i).Cells(8).Text.Replace("'", "''") & "','" & myGridView.Rows(i).Cells(5).Text.Replace("'", "''") & "','" & myGridView.Rows(i).Cells(6).Text.Replace("'", "''") & "','" & myGridView.Rows(i).Cells(7).Text.Replace("'", "''") & "'"
                    mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
...
#End Region
#Region "Sub & Function"
    Private Function ValidateCHK(ByVal Status As String, ByVal oldfrom As String, ByVal oldto As String) As Boolean
        ssql2 = "Exec sp_pr_insupddelchkEmpBenefit '" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "','" & txtOCP_ID_SALARY.Text.Replace("'", "''") & "','" & txtDATE_FROM.Text & "','" & txtDATE_TO.Text & _
        "','" & oldfrom & "','" & oldto & "','" & Session("Company") & "','Check','" & Status & "','" & ddlOPTION_PAY_CYCLE.SelectedValue & "','" & txtVALUE.Text & "','" & ddlOPTION_PAY_TYPE.SelectedValue & "','" & ddlOPTION_CALCULATE_ON.SelectedValue & "'"
        myDS1 = mySQL.ExecuteSQL(ssql2, Session("Company"), Session("EmpID"))

        If myDS1.Tables.Count > 0 Then
            If myDS1.Tables(0).Rows.Count > 0 Then
                ShowMessage("Duplicate Record Found!")

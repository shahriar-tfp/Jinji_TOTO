...
Partial Class Pages_Payroll_Employee_Payslip
...
#End Region
    ...
    Private Sub BindGrid()
        Try
            ssql = "Exec sp_pr_SelEmpPayrollDetails N'" & Session("Company").ToString & "',N'" & _
                            Session("Module").ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text.ToString.Replace("'", "''") & "',N'" & _
    ...
    Protected Sub imgbtnDelete_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnDelete.Click
        ...
        If chkvalidate() Then
            ssql12 = "exec sp_pr_ChkDelPayslip '" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "','" & ddlMONTH.SelectedValue & "','" & _
            ddlYEAR.SelectedValue & "','" & ddlOPTION_PAY_CYCLE.SelectedValue & "','DEL'"
            mySQL.ExecuteSQL(ssql12)
            Dim strScript As String = "<script language=" & """" & "javascript" & """" & ">"
            strScript &= "alert('Delete Sucessfully!');"
            strScript &= "<" & "/script>"
            Page.ClientScript.RegisterClientScriptBlock(Me.GetType, "Delete Sucessfully", strScript)
            imgbtnSearch_Click(Nothing, Nothing)
        End If
    End Sub

    Public Function chkvalidate() As Boolean
        chkvalidate = False
        Dim stryear As String, strmonth As String
        Dim strScript As String = "<script language=" & """" & "javascript" & """" & ">"

        Dim ssql10 As String, ssql11 As String, DDate As String, myds10 As DataSet, myds11 As DataSet
        ...
        If Len(strmonth) = 1 Then
            strmonth = "0" + strmonth
        End If
        DDate = stryear + strmonth + "01000000"

        ssql10 = "Exec sp_sa_chkTransactionLockDate """ & Session("Company").ToString & """,""" _
            & Session("EmpID") & """,""" & DDate & """," & 1 & "," & 0
        myds10 = mySQL.ExecuteSQL(ssql10)

        If myds10.Tables.Count > 0 Then
            If myds10.Tables(0).Rows.Count > 0 Then
                If myds10.Tables(0).Rows(0).Item(0).ToString = "0" Then
                    strScript &= "alert('You are not allow to delete record because effective date has been locked or expired!');"
                    strScript &= "<" & "/script>"
                    Page.ClientScript.RegisterClientScriptBlock(Me.GetType, "Error", strScript)
                    'MsgBox("You are not allow to delete record because effective date has been locked or expired!")
                    Exit Function
                End If
            End If
        End If
        ssql11 = "exec sp_pr_ChkDelPayslip '" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "','" & ddlMONTH.SelectedValue & "','" & _
        ddlYEAR.SelectedValue & "','" & ddlOPTION_PAY_CYCLE.SelectedValue & "','CHECK'"
        myds11 = mySQL.ExecuteSQL(ssql11)

        If myds11.Tables.Count > 0 Then
            If myds11.Tables(0).Rows.Count > 0 Then
                strScript &= "alert('Delete Not Allowed! Got Next Month Payroll Record!');"
                strScript &= "<" & "/script>"
                Page.ClientScript.RegisterClientScriptBlock(Me.GetType, "Error", strScript)
                '                MsgBox("Delete Not Allowed! Got Next Month Payroll Record!")
                Exit Function
            End If
        End If

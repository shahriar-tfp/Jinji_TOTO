...
Partial Class Pages_Payroll_Publish_Retro
    ...
    Private Sub PagePreload()
        ...
        'get Page Title
        ssql = "exec sp_sa_GetPageTitle 'PUBLISH_RETRO'"
        myDS = New DataSet
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
    ...
    Private Sub BindGrid()
        Try
            ...
            End If
            If SearchCriteria = False Then
                ssql = "Exec sp_pr_selPublishRetro N'" & Session("Company").ToString & "',N'" & ddlYEAR.SelectedValue & "',N'" & ddlMONTH.SelectedValue & "',N'" & ddlFILTER_BY.SelectedValue & "','" & Session("EmpID").ToString & "'"
                'Else
            ...
            If Validate1() Then
                For i = 0 To myGridView.Rows.Count - 1
                    ...
                    If chkBox.Checked = True Then
                        ...
                        ssql1 = "select isnull(dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & myGridView.Rows(i).Cells(1).Text.ToString.Replace("&amp;", "&").Replace("'", "''") & "'),''),isnull(dbo.fn_getcomocpidbycodename(N'" & myGridView.Rows(i).Cells(5).Text.ToString.Replace("&amp;", "&") & "','SALARY','" & Session("Company").ToString & "'),'')"
    ...
    Private Function Validate1() As Boolean
        If ddlPayCycle.SelectedValue = "" Then
            ShowMessage("Cannot be Blank! Payment Cycle")
            ddlPayCycle.Focus()
            Return False
        End If
        Return True

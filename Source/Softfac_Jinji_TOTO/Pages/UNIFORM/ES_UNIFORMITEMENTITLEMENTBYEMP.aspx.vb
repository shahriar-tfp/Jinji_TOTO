...
Partial Class PAGES_UNIFORM_ES_UNIFORMITEMENTITLEMENTBYEMP
...
#End Region
    ...
    End Sub
    Protected Sub txtCATEGORYID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCATEGORYID.TextChanged
        Dim code As String

        Dim str As String
        myDS = mySQL.ExecuteSQL("select dbo.fn_GetCode_ES_UNIFORMITEMCATEGORY_Vw('" & txtCATEGORYID.Text.Replace("'", "''") & "')")
        code = myDS.Tables(0).Rows(0).Item(0).ToString

        mySetting.GetLookupValue_ImageButton(imgbtnITEMID, Form.ID, "txtITEMID", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "ITEMID" & """," & """" & """," & """" & Session("EmpID").ToString & """,""" & code & """")

    End Sub

    Protected Sub txtITEMID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtITEMID.TextChanged
        Dim code As String

        ssql = "select dbo.fn_GetCode_ES_UNIFORMITEM_Vw('" & txtITEMID.Text.Replace("'", "''") & "')"
        myDS = mySQL.ExecuteSQL(ssql)
        code = myDS.Tables(0).Rows(0).Item(0).ToString

        mySetting.GetLookupValue_ImageButton(imgbtnSIZE, Form.ID, "txtSIZE", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "SIZE" & """," & """" & """," & """" & Session("EmpID").ToString & """,""" & code & """")

...
#Region "Panel Edit"
    ...
    Protected Sub imgbtnGROUP_CODE2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnGROUP_CODE2.Click
        ...
        lstleft.Items.Clear()
        lstright.Items.Clear()
        ssql = "Exec sp_sa_getListBoxValue 'Payslip','" & Session("Company") & "','" & ddlOPTION_GROUP.SelectedValue & "','" & Session("EmpID").ToString & "','" & ddlOPTION_PAY_CYCLE.SelectedValue & "','','',''"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "MONTH" Then
                    If lblMONTH.Visible = True And ddlMONTH.Visible = True Then
                        autonum += 1
                        imgKeyMONTH.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyMONTH.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyMONTH.Style.Add("position", "absolute")
                        autonum += 1
                        lblMONTH.CssClass = "wordstyle12"
                        lblMONTH.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblMONTH.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblMONTH.Style.Add("position", "absolute")
                        autonum += 1
                        ddlMONTH.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        ddlMONTH.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ddlMONTH.Style.Add("position", "absolute")
                        ddlMONTH.Width = txtWidth
                        autonum += 1
                        imgbtnMONTH.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnMONTH.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnMONTH.Style.Add("position", "absolute")
                    End If
                End If
...
#End Region
        ...
        Try
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    ...
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&spid=" & spid & "&Year=" & ddlYEAR.SelectedValue & "&Month=" & ddlMONTH.SelectedValue & _
                    "&Option_Pay_Cycle=" & ddlOPTION_PAY_CYCLE.SelectedValue & "&Paytype=" & ddlOPTION_PAY_TYPE.SelectedValue & "&User_Profile_Code=" & Session("EmpID").ToString & "&ByType=" & ddlOPTION_GROUP.SelectedValue & "&rdTemplate=Roster_AttSummary2"
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        ...
        ElseIf ddlOPTION_PAY_TYPE.SelectedValue = "" Then
            ...
            Return False
        ElseIf ddlMONTH.SelectedValue = "" Then
            lblresult2.Text = "[" & lblMONTH.Text & "] Is A Required Field!"
            ddlMONTH.Focus()
            Return False

...
Partial Class Pages_Reports_Report_TimeSheet
...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        ssql = "Select Security_Role_Profile_Code from user_profile where code = '" & Session("EmpID").ToString & "'"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

        If myDS.Tables(0).Rows.Count > 0 Then
            If myDS.Tables(0).Rows(0).Item(0).ToString = "EMP" Then
                lblOPTION_RESIGN.Visible = False
                ddlOPTION_RESIGN.Visible = False
                imgKeyOPTION_RESIGN.Visible = False
            End If
        End If
        ...
        mySetting.GetBtnImgUrl(imgBtnPrint, Session("Company").ToString, btnColourDef, "btnPreview.png")
        ...
        If ddlOPTION_RESIGN.SelectedIndex = 0 Then
            ddlOPTION_RESIGN.SelectedValue = "ALL"
        End If
        If ddlOPTION_GROUP.SelectedIndex = 0 Then
            ddlOPTION_GROUP.SelectedValue = "ADD"
        End If
    End Sub
...
#Region "Panel Edit"
    ...
    Protected Sub imgbtnGROUP_CODE2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnGROUP_CODE2.Click
        ...
        lstleft.Items.Clear()
        lstright.Items.Clear()
        ssql = "Exec sp_sa_getListBoxValue 'TimeSheet','" & Session("Company") & "','" & ddlOPTION_GROUP.SelectedValue & "','" & Session("EmpID").ToString & "','" & ddlOPTION_RESIGN.SelectedValue & "','','',''"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "OPTION_RESIGN" Then
                    If lblOPTION_RESIGN.Visible = True And ddlOPTION_RESIGN.Visible = True Then
                        autonum += 1
                        imgKeyOPTION_RESIGN.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyOPTION_RESIGN.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyOPTION_RESIGN.Style.Add("position", "absolute")
                        autonum += 1
                        lblOPTION_RESIGN.CssClass = "wordstyle12"
                        lblOPTION_RESIGN.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblOPTION_RESIGN.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblOPTION_RESIGN.Style.Add("position", "absolute")
                        autonum += 1
                        ddlOPTION_RESIGN.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        ddlOPTION_RESIGN.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ddlOPTION_RESIGN.Style.Add("position", "absolute")
                        ddlOPTION_RESIGN.Width = ddlWidth
                        autonum += 1
                        imgbtnOPTION_RESIGN.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnOPTION_RESIGN.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnOPTION_RESIGN.Style.Add("position", "absolute")
                    End If
                End If
...
#End Region
    ...
    Protected Sub ddlOPTION_RESIGN_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlOPTION_RESIGN.SelectedIndexChanged
        If lstleft.Visible = True And imgbtnGROUP_CODE2.Visible = True Then
            lstleft.Items.Clear()
            lstright.Items.Clear()
            ssql = "Exec sp_sa_getListBoxValue 'TimeSheet','" & Session("Company") & "','" & ddlOPTION_GROUP.SelectedValue & "','" & Session("EmpID").ToString & "','" & ddlOPTION_RESIGN.SelectedValue & "','','',''"
            myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

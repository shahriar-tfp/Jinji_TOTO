...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myTargetURL = myTargetURL & "&OrganID=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&SPID=" & myDS.Tables(0).Rows(0).Item(3).ToString & "&Type=ADD&paytype=" & ddlOPTION_PAY_TYPE.SelectedValue & "&Location=" & ddlOPTION_LOCATION.SelectedValue & "&Module=" & Session("Module").ToString & "&userID=" & Session("EmpID").ToString & "&Cheque=" & txtCHEQUE.Text.ToUpper & "&PICName=" & txtPIC_NAME.Text.ToUpper & "&PICNRIC=" & txtPIC_NRIC.Text.ToUpper & "&PICPosition=" & txtPIC_POSITION.Text.ToUpper & "&PICTelNo=" & txtPIC_TELNO.Text.ToUpper & "&Page=" & ddlOPTION_PAGE.SelectedValue & "&rdTemplate=pr_GovEPF"
    ...
    Protected Sub imgBtnSaveAs_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSaveAs.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ssql1 = "Exec sp_pr_SelEPF 'EPFDETAIL','" & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & "," & myDS.Tables(0).Rows(0).Item(2).ToString & ",'" & txtPIC_NAME.Text.Replace("'", "''") & "'"
                myDS1 = New DataSet
    ...
    Protected Sub imgBtnPrintBBCD_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrintBBCD.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&Option_Location=" & ddlOPTION_LOCATION.SelectedValue & "&Cheque=" & txtCHEQUE.Text.ToUpper & "&PICName=" & txtPIC_NAME.Text.ToUpper & "&PICNRIC=" & txtPIC_NRIC.Text.ToUpper & "&PICPosition=" & txtPIC_POSITION.Text.ToUpper & "&PICTelNo=" & txtPIC_TELNO.Text.ToUpper & "&rdTemplate=pr_GovBBCD"
                Response.Write("<script>window.open('" & myTargetURL & "');</script>")
            End If
        End If
        ssql = Nothing

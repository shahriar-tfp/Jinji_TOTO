...
Partial Class Pages_Reports_Report_OT_Analysis
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            ssql = "select dbo.fn_UnDisplayDate('" & txtDATE_FROM.Text & "','" & Session("Company").ToString() & "','" & Session("Module").ToString & "'),dbo.fn_UnDisplayDate('" & txtDATE_TO.Text & "','" & Session("Company").ToString() & "','" & Session("Module").ToString & "'),@@spid"
            myDS = New DataSet()
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    ...
                    For k = 0 To itemCount - 1
                        ...
                        ssql1 = Nothing
                    Next
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&FromDate=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&ToDate=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&spid=" & spid & "&User_Profile_Code=" & Session("EmpID").ToString & "&Group=" & ddlOPTION_GROUP.SelectedValue & "&rdTemplate=Roster_OTAnalysis"
                End If

...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            ssql = "select dbo.fn_GetEmpID(N'" & Session("EmpID").ToString & "'),'" & ddlOPTION_MONTH.SelectedValue & "','" & ddlOPTION_YEAR.SelectedValue & "',@@spid"
            myDS = New DataSet()
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    ...
                    ssql = "exec sp_pr_RptCusEPayslip_Kajima '" & Session("Company").ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','','','ADD','" & Session("EmpID").ToString & "','MONTHLY'"

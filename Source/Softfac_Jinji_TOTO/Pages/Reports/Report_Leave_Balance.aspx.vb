...
Partial Class Pages_Reports_Report_Leave_Balance
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ds = New DataSet()
        Dim xtr As New System.Xml.XmlTextReader(Server.MapPath("~") & "\_Templates\Ar\ls_leaveummary2.rpx")
	...
	'Dim path As String = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)
        ...
        ssql = "select dbo.fn_GetOCPIDbyCODENAME(N'" & txtOCP_ID_LEAVE.Text & "','LEAVE'),dbo.fn_undisplaydate(N'" & txtDATE_FROM.Text & "','" & Session("Company").ToString & "','" & Session("Module") & "'),@@spid"
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ...
                ssql = "Exec [sp_ls_RptLeaveBalance] '" & Session("Company").ToString & "','" & Session("EmpID").ToString & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & _
                myDS.Tables(0).Rows(0).Item(1).ToString & "','" & ddlOPTION_GROUP.SelectedValue & "'," & spid
                xtr = New System.Xml.XmlTextReader(Server.MapPath("~") & "\_Templates\Ar\ls_leaveSummary2.rpx")
 		...
 		'rpx.Run() ' This is where i add 
              	...
              	'Dim XlsExport1 As New GrapeCity.ActiveReports.Export.Excel.Section.XlsExport() ' This is where i add 
               	'XlsExport1.Export(rpx.Document, path + "\LeaveBalance" + String.Format("{0:ddMMyyyyHHmmss}", DateTime.Now) + ".xls") ' This is where i add and i need this to download in their local and show the notification on website

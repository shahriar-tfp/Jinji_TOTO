...
Partial Class Pages_Reports_Report_Medical_ByMonthYear
...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "Panel_Doctor" & """," & """" & """," & """" & Session("EmpID").ToString & """"
        mySetting.GetLookupValue_ImageButton(imgbtnPanel_Doctor, Form.ID, "txtPanel_Doctor", "CodeName", ssql)
        ...
        If myDS.Tables.Count > 1 Then
            ...
            If myDT2.Rows.Count > 0 Then
                ...
                ddlMonthssql = "Create Table #Result([Code] nvarchar(50), [Name] nvarchar(255)); Insert Into #Result Select '','';Insert Into #Result Select '00','ALL';"
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "PANEL_DOCTOR" Then
                    If lblPanel_Doctor.Visible = True And txtPanel_Doctor.Visible = True Then
                        autonum += 1
                        imgKeyPanel_Doctor.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyPanel_Doctor.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyPanel_Doctor.Style.Add("position", "absolute")
                        autonum += 1
                        lblPanel_Doctor.CssClass = "wordstyle12"
                        lblPanel_Doctor.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblPanel_Doctor.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblPanel_Doctor.Style.Add("position", "absolute")
                        autonum += 1
                        txtPanel_Doctor.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        txtPanel_Doctor.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        txtPanel_Doctor.Style.Add("position", "absolute")
                        txtPanel_Doctor.Width = txtWidth
                        autonum += 1
                        imgbtnPanel_Doctor.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnPanel_Doctor.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnPanel_Doctor.Style.Add("position", "absolute")
                    End If
                End If
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            If txtPanel_Doctor.Text = "" Then
                ssql = "select dbo.fn_GetEmpID(N'" & Session("EmpID").ToString & "'),'" & ddlOPTION_MONTH.Text & "','" & ddlOPTION_YEAR.Text & "',@@spid,''"
            Else
                ssql = "select dbo.fn_GetEmpID(N'" & Session("EmpID").ToString & "'),'" & ddlOPTION_MONTH.Text & "','" & ddlOPTION_YEAR.Text & "',@@spid,dbo.fn_GetCode_MEDICAL_PANELDOCTOR_Vw('" & txtPanel_Doctor.Text & "')"
            End If
            ...
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&Month=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&User_Profile_Code=" & Session("EmpID").ToString & "&PanelDoctor=" & myDS.Tables(0).Rows(0).Item(4).ToString & "&rdTemplate=" & ddlOPTION_TYPE.SelectedValue

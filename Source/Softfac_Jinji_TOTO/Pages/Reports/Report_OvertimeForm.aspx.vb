...
Partial Class Pages_Reports_Report_OvertimeForm
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_UnDisplayDate('" & txtYEAR_MONTH.Text & "',dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),'" & Session("Module").ToString & "'),@@spid"
        myDS = New DataSet()
        myDS = mySQL.ExecuteSQL(ssql)
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                'myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Date=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&FormID=" & ddlFORMID.SelectedValue & "&rdTemplate=Roster_OTFormDetails"
                'Response.Write("<script>window.open('" & myTargetURL & "');</script>")
                'myTargetURL = clsRpt.GetPDFReportURL()
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Date=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&FormID=" & ddlFORMID.SelectedValue & "&rdTemplate=Roster_OTFormSummary"
                Response.Write("<script>window.open('" & myTargetURL & "');</script>")
            End If
        End If
        ssql = Nothing
    ...
    Public Sub getFormID()

        Dim ddlFORMIDssql As String, myDS5 As DataSet

        If txtYEAR_MONTH.Text <> "" Then
            ddlFORMIDssql = "Create Table #Result([Code] nvarchar(50), [Name] nvarchar(255)); Insert Into #Result Select '','';"
            ddlFORMIDssql = ddlFORMIDssql + "Insert Into #Result Select distinct formid,formid from PREAPPROVED_OVERTIME where date ='" & mySetting.ConvertDateToDecimal(txtYEAR_MONTH.Text, Session("Company").ToString, Session("Module").ToString) & "' and user_profile_code_create = '" & Session("EmpID").ToString & "';"
            ddlFORMIDssql = ddlFORMIDssql + " Select * From #Result; Drop Table #Result"
        Else
            ddlFORMIDssql = "Create Table #Result([Code] nvarchar(50), [Name] nvarchar(255)); Insert Into #Result Select '','';"
            ddlFORMIDssql = ddlFORMIDssql + "Insert Into #Result Select distinct formid,formid from PREAPPROVED_OVERTIME where user_profile_code_create = '" & Session("EmpID").ToString & "';"
            ddlFORMIDssql = ddlFORMIDssql + " Select * From #Result; Drop Table #Result"
        End If
        myDS5 = mySQL.ExecuteSQL(ddlFORMIDssql)
        If myDS5.Tables.Count > 0 Then
            If myDS5.Tables(0).Columns.Count > 1 Then
                ddlFORMID.DataTextField = "Name"
                ddlFORMID.DataValueField = "Code"
                ddlFORMID.DataSource = myDS5
                ddlFORMID.DataBind()
            End If
        End If
        myDS5 = Nothing
    ...
    Protected Sub txtYEAR_MONTH_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtYEAR_MONTH.TextChanged
        getFormID()
    End Sub

    Protected Sub ddlFORMID_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlFORMID.SelectedIndexChanged
        filldate()
    End Sub
    ...
    Private Sub filldate()
        ...
        fordate = "Create Table #Result([Code] nvarchar(50), [Name] nvarchar(255));"
        fordate = fordate + "Insert Into #Result Select distinct date,dbo.fn_displaydate(date,company_profile_code,'ROSTER') from PREAPPROVED_OVERTIME where formid = '" & ddlFORMID.SelectedValue & "';"
        fordate = fordate + " Select * From #Result; Drop Table #Result"
        myDS2 = mySQL.ExecuteSQL(fordate)
        If myDS2.Tables.Count > 0 Then
            If myDS2.Tables(0).Columns.Count > 1 Then
                txtYEAR_MONTH.Text = myDS2.Tables(0).Rows(0).Item(1).ToString
            End If
        End If
    End Sub
End Class

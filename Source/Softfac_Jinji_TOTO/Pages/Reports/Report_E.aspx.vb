    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&PicID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&rdTemplate=pr_GovEForm"
                Response.Write("<script>window.open('" & myTargetURL & "');</script>")
                myTargetURL = clsRpt.GetPDFReportURL()
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&PicID=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&rdTemplate=pr_GovCP8D"
                Response.Write("<script>window.open('" & myTargetURL & "');</script>")
            End If
        End If
        ssql = Nothing
    ...
    Protected Sub imgBtnSaveAs_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSaveAs.Click
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                myDS1 = Nothing
                ssql1 = Nothing
                ssql2 = "Exec [sp_pr_rptE8D] '" & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & ",'" & myDS.Tables(0).Rows(0).Item(2).ToString & "','','TXT1'"
                myDS1 = New DataSet
                ...
                myDS1 = Nothing
                ssql1 = Nothing
                ssql1 = "Exec [sp_pr_rptE8D] '" & myDS.Tables(0).Rows(0).Item(0).ToString & "'," & myDS.Tables(0).Rows(0).Item(1).ToString & ",'" & myDS.Tables(0).Rows(0).Item(2).ToString & "','','TXT2'"
                myDS1 = New DataSet
                myDS1 = mySQL.ExecuteSQL(ssql1)
                For i = 0 To myDS1.Tables(0).Rows.Count - 1
                    str.Append(myDS1.Tables(0).Rows(i).Item(0).ToString)

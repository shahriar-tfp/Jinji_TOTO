...
Partial Class Pages_Reports_Report_Appraisal_Letter
...
#End Region
    ...
    Sub PagePreload()
        ...
        If myDS.Tables.Count > 1 Then
            ...
            If myDT2.Rows.Count > 0 Then
                ...
                ssql = "Exec sp_ap_sel_AppraisalPeriod '','','" & "AllPeriod" & "'"
                Dim myDS5 As DataSet

                myDS5 = mySQL.ExecuteSQL(ssql)
                If myDS5.Tables.Count > 0 Then
                    If myDS5.Tables(0).Columns.Count > 1 Then
                        ddlOPTION_APPRAISALPERIOD.DataTextField = "Desc"
                        ddlOPTION_APPRAISALPERIOD.DataValueField = "Period"
                        ddlOPTION_APPRAISALPERIOD.DataSource = myDS5
                        ddlOPTION_APPRAISALPERIOD.DataBind()
                    End If
                End If
                myDS5 = Nothing
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),@@spid"
        myDS = New DataSet()
        ...
        If ValidatePrint(myDS) = True Then
            If myDS.Tables(0).Rows.Count = 1 Then
                ...
                myTargetURL = myTargetURL & "&Company_Profile_Code=" & myDS.Tables(0).Rows(0).Item(0).ToString & "&User_Profile_Code=" & Session("EmpID").ToString & "&Period=" & ddlOPTION_APPRAISALPERIOD.SelectedValue & "&Spid=" & spid & "&rdTemplate=" & ddlOPTION_REPORT.SelectedValue
    ...
    Sub AutoAdjustPosition(ByVal strSelect As String)
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "OPTION_APPRAISALPERIOD" Then
                    If lblOPTION_APPRAISALPERIOD.Visible = True And ddlOPTION_APPRAISALPERIOD.Visible = True Then
                        autonum += 1
                        imgKeyOPTION_APPRAISALPERIOD.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyOPTION_APPRAISALPERIOD.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyOPTION_APPRAISALPERIOD.Style.Add("position", "absolute")
                        autonum += 1
                        lblOPTION_APPRAISALPERIOD.CssClass = "wordstyle12"
                        lblOPTION_APPRAISALPERIOD.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblOPTION_APPRAISALPERIOD.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblOPTION_APPRAISALPERIOD.Style.Add("position", "absolute")
                        autonum += 1
                        ddlOPTION_APPRAISALPERIOD.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        ddlOPTION_APPRAISALPERIOD.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        ddlOPTION_APPRAISALPERIOD.Style.Add("position", "absolute")
                        ddlOPTION_APPRAISALPERIOD.Width = ddlWidth
                        autonum += 1
                        imgbtnOPTION_APPRAISALPERIOD.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnOPTION_APPRAISALPERIOD.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnOPTION_APPRAISALPERIOD.Style.Add("position", "absolute")
                    End If
                End If
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        If myds.Tables(0).Rows(0).Item(0).ToString = "" Then
            ...
            Return False
        ElseIf ddlOPTION_APPRAISALPERIOD.SelectedValue = "" Then
            lblresult2.Text = "Invalid Input Date Format For [" & lblOPTION_APPRAISALPERIOD.Text & "] !"
            ddlOPTION_APPRAISALPERIOD.Focus()
            Return False
        ElseIf ddlOPTION_REPORT.SelectedValue = "" Then
            lblresult2.Text = "Invalid Input Date Format For [" & lblOPTION_REPORT.Text & "] !"

...
Partial Class Pages_Reports_REPORT_MEDICAL_BYYEAR
            ...
            For i = 0 To myDS.Tables(0).Rows.Count - 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "OPTION_CATEGORY" Then
                    If lblOPTION_CATEGORY.Visible = True And ddlOPTION_CATEGORY.Visible = True Then
                        ...
                        ddlOPTION_CATEGORY.Width = txtWidth
                        autonum += 1
                ...
                If UCase(myDS.Tables(0).Rows(i).Item(1).ToString) = "MIN_DAY" Then
                    If lblMIN_DAY.Visible = True And txtMIN_DAY.Visible = True Then
                        autonum += 1
                        imgKeyMIN_DAY.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgKeyMIN_DAY.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgKeyMIN_DAY.Style.Add("position", "absolute")
                        autonum += 1
                        lblMIN_DAY.CssClass = "wordstyle12"
                        lblMIN_DAY.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        lblMIN_DAY.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        lblMIN_DAY.Style.Add("position", "absolute")
                        autonum += 1
                        txtMIN_DAY.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        txtMIN_DAY.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        txtMIN_DAY.Style.Add("position", "absolute")
                        txtMIN_DAY.Width = txtWidth
                        autonum += 1
                        imgbtnMIN_DAY.Style.Add("left", mySetting.GetObjPosition(EctWidth, autonum, "X"))
                        imgbtnMIN_DAY.Style.Add("top", mySetting.GetObjPosition(EctWidth, autonum, "Y"))
                        imgbtnMIN_DAY.Style.Add("position", "absolute")
                    End If
                End If
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        Try
            ...
            ssql = "select dbo.fn_GetEmpID(N'" & Session("EmpID").ToString & "'),'" & ddlOPTION_CATEGORY.Text & "','" & ddlOPTION_YEAR.Text & "',@@spid"
            myDS = New DataSet()
            myDS = mySQL.ExecuteSQL(ssql)
            If ValidatePrint(myDS) = True Then
                If myDS.Tables(0).Rows.Count = 1 Then
                    myTargetURL = myTargetURL & "&Company_Profile_Code=" & Session("Company").ToString & "&Category=" & myDS.Tables(0).Rows(0).Item(1).ToString & "&Year=" & myDS.Tables(0).Rows(0).Item(2).ToString & "&User_Profile_Code=" & Session("EmpID").ToString & "&minday=" & txtMIN_DAY.Text & "&rdTemplate=" & ddlOPTION_TYPE.SelectedValue

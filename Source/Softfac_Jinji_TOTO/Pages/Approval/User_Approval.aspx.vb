...
#Region "Panel Edit"
    ...
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ssql1 = "select dbo.fn_ReturnCompanyProfileCode('" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_ReturnModuleProfileCodeByCodeName('" & txtMODULE_PROFILE_CODE.Text & "'),dbo.fn_GetUserProfileCode('" & txtUSER_PROFILE_CODE.Text.Replace("'", "''") & "')"
    ...
    Protected Sub imgbtnEMPLOYEE_PROFILE_ID2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnEMPLOYEE_PROFILE_ID2.Click
        lstleft.Items.Clear()
        lstright.Items.Clear()
        ssql = "Exec sp_sa_getListBoxValue 'getGroupByOCP','" & Session("Company") & "','" & ddlOPTION_TYPE.SelectedValue & "','" & ddlOCP_ID.SelectedValue & "','" & txtMODULE_PROFILE_CODE.Text & "','" & txtAPPROVAL_LEVEL.Text & "','" & txtUSER_PROFILE_CODE.Text.Replace("'", "''") & "',''"
        myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        If myDS.Tables(0).Rows.Count > 0 Then
...
#Region "Sub & Function"
    ...
    Private Function ValidateInsertUpdate() As Boolean
        ...
        Try
            ...
            For k = 0 To itemCount - 1
                ssql1 = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text.Replace("'", "''") & "'),dbo.fn_GetCode_Module_Profile_Vw(N'" & txtMODULE_PROFILE_CODE.Text & "'),isnull(dbo.fn_GetCode_User_Profile_Vw(N'" & txtUSER_PROFILE_CODE.Text & "'),''),ISNULL(dbo.fn_ReturnEmpID(N'" & lstright.Items(k).Value.Replace("'", "''") & "'),'')"

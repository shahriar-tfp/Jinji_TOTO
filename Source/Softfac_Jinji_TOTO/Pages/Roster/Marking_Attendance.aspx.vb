...
Partial Class Pages_Roster_Marking_Attendance
...
#Region "Public Declaration"
    ...
    Public CurrentDate, yesterday, twodaybefore, twoweekbefore As String
#End Region
...
#Region "Private Declaration"
    ...
    Dim chkSelected, chkSelected1, chkSelected2, chkSelected3, chkSelected4 As CheckBox
    ...
    Dim Action, DateApplyOn, DateApplyFor, EmpID, EmpCode, Leave, Period, TimeIn, TimeOut As String
    Dim workingdate, date_time, errorcode, leaveCode, inout, tmsid, daytypeid, shiftid As String
...
#Region "Page Setting"
    ...
    Sub PagePreload()
        ...
        Try
            ...
            yesterday = mySQL.fn_DateAdd("day", "-1", CurrentDate)
            twodaybefore = mySQL.fn_DateAdd("day", "-2", CurrentDate)
            twoweekbefore = mySQL.fn_DateAdd("day", "-14", CurrentDate)
            ...
            'get chk sel name
            chkAAll.Text = "Approve All In"
            chkRAll.Text = "Reject All In"
            chkAOAll.Text = "Approve All Out"
            chkROAll.Text = "Reject All Out"
...
#Region "Panel Action"
    ...
    End Sub
    Protected Sub lnkBtnViewYesterday_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkBtnViewYesterday.Click
        lnkBtnViewYesterday_Click()
    End Sub
    ...
    Protected Sub chkAOAll_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkAOAll.CheckedChanged

        chkROAll.Checked = False
        OptionCheckedChange3()
    ...
    Protected Sub chkROAll_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkROAll.CheckedChanged

        chkAOAll.Checked = False
        OptionCheckedChange3()
    ...
    Protected Sub ddlOption_Type_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlOption_Type.SelectedIndexChanged
        ...
        Dim SELECTAID As DataSet

        sqltext = ddlOption_Type.SelectedValue.ToUpper
        If ddlOption_Type.SelectedValue.ToUpper = "SUPERVISOR" Then
            ssql = "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & ddlOption_Type.SelectedValue & """,''," & """" & Session("EmpID").ToString & """"
            txtAID.Visible = False
            ddlSelectAID.Visible = True
            imgBtnAID.Visible = False
            SELECTAID = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)

            If SELECTAID.Tables.Count > 0 Then
                If SELECTAID.Tables(0).Columns.Count > 1 Then
                    ddlSelectAID.DataTextField = "CodeName"
                    ddlSelectAID.DataValueField = "CodeName"
                    ddlSelectAID.DataSource = SELECTAID
                    ddlSelectAID.DataBind()
                    txtAID.Text = ddlSelectAID.SelectedValue
                End If
            End If
            SELECTAID = Nothing
        Else
    ...
    Protected Sub imgBtnCancel_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnCancel.Click
        ...
        lblresult.Text = ""
        If Session("TabStatus") = "TODAY" Then
            lnkBtnCloseToday_Click()
        ElseIf Session("TabStatus") = "YES" Then
            lnkBtnCloseYesterday_Click()
        ElseIf Session("TabStatus") = "APPROVED" Then
            lnkBtnCloseApproved_Click()
        ElseIf Session("TabStatus") = "ALL" Then
            lnkBtnCloseALL_Click()
        End If
...
#Region "Sub & Function"
    ...
    Public Sub SubLoadSSQL(ByVal Status As String)
        ...
        'ssql = "Select dbo.fn_DateAdd('Day',-1,'" & Session("CurrentDate") & "'),dbo.fn_DateAdd('Day',-14,'" & Session("CurrentDate") & "'),dbo.fn_DateAdd('Day',-2,'" & Session("CurrentDate") & "')"
        'myDS1 = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
        'If myDS1.Tables(0).Rows.Count > 0 Then
        If Status = "YES" And Session("action") = "PENDING" Then
            FromDate = yesterday
            ToDate = yesterday
            Status = "PENDING"
        ElseIf Status = "PENDING" And Session("action") = "PENDING" Then
            FromDate = "19000101000000"
            ToDate = "19000101000000"
        ElseIf Status = "ALL" And Session("action") = "PENDING" Then
            FromDate = twoweekbefore
            ToDate = CurrentDate
        ElseIf Status = "APPROVED" Then
            FromDate = twodaybefore
            ToDate = CurrentDate
        Else
            FromDate = "19000101000000"
            ToDate = "19000101000000"
        ...
        ssql = "Exec sp_tms_MarkingAttendance '" & Session("Company") & "','" & Session("EmpID") & "','','" & Status & _
    ...
    Public Sub RetrieveDetail()
        ...
        If myDS.Tables.Count = 3 Then
            If myDS.Tables(1).Rows.Count = 0 Then
                ...
                chkAOAll.Enabled = False
                chkRAll.Enabled = False
                chkROAll.Enabled = False
                Exit Sub
            Else
                ...
                chkAOAll.Enabled = True
                chkRAll.Enabled = True
                chkROAll.Enabled = True
            ...
            'Dim i As Integer
            'Dim gridrows As GridViewRow

            'i = 6
            'For Each gridrows In myGridView1.Rows
            '    gridrows.Cells(i).Controls.Add(New CheckBox())
            'Next
    ...
    Public Sub SearchDetail(ByVal Status As String)
        ...
        If mySetting.CheckTextNull(txtADateFrom.Text) = False Then
            ssql = "Select dbo.fn_DateAdd('Day',-1,'" & Session("CurrentDate") & "'),dbo.fn_DateAdd('Day',-14,'" & Session("CurrentDate") & "')"
            myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            If myDS.Tables(0).Rows.Count > 0 Then
                If Status = "YES" Then
                    FromDate = myDS.Tables(0).Rows(0).Item(0).ToString
                    ToDate = myDS.Tables(0).Rows(0).Item(0).ToString
                    Status = "PENDING"
                ElseIf Status = "PENDING" Then
                    FromDate = "19000101000000"
                    ToDate = "19000101000000"
                Else
                    FromDate = myDS.Tables(0).Rows(0).Item(1).ToString
        ...
        Else
            ...
            If ddlOption_Type.SelectedValue = "SUPERVISOR" And chkADepartment.Checked = True Then
                txtAID.Text = ddlSelectAID.SelectedValue
            End If
            If txtAID.Text <> "" Then
                ssql = "Select ID From Organisation_Code_Profile_Vw Where Company_Profile_Code = '" & Session("Company") & _
                       "' And Option_Type = '" & ddlOption_Type.SelectedValue & "' And CodeName = '" & txtAID.Text.Replace("'", "''") & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                If myDS.Tables(0).Rows.Count > 0 Then
                    vID = myDS.Tables(0).Rows(0).Item(0).ToString & "%"
                End If
            ...
            If vType = "SUPERVISOR" Then
                UserID = mySetting.GetEmployeeCodeByCodeName_ReturnString(Session("Company"), vID.Replace("'", "''"))
                vID = "%"
            End If
            ssql = "Exec sp_tms_MarkingAttendance '" & Session("Company") & "','" & UserID & "','','" & Status & _
    ...
    Private Sub UpdPendingLeaveToAppLeave()
        ...
        For i = 0 To myGridView1.Rows.Count - 1
            Dim chkCursor As CheckBox = myGridView1.Rows(i).Cells(0).Controls(1)
            If chkCursor.Checked = True Then
                workingdate = Trim(myGridview2.Rows(i).Cells(2).Text)
                EmpID = Trim(myGridview2.Rows(i).Cells(3).Text)
                date_time = Trim(myGridview2.Rows(i).Cells(4).Text).Replace("&nbsp;", "")
                errorcode = Trim(myGridview2.Rows(i).Cells(5).Text).Replace("&nbsp;", "")
                leaveCode = Trim(myGridview2.Rows(i).Cells(12).Text).Replace("&nbsp;", "")
                tmsid = Trim(myGridview2.Rows(i).Cells(13).Text).Replace("&nbsp;", "")
                daytypeid = Trim(myGridview2.Rows(i).Cells(14).Text).Replace("&nbsp;", "")
                shiftid = Trim(myGridview2.Rows(i).Cells(15).Text).Replace("&nbsp;", "")

                'leave approved
                ssql = "Exec sp_tms_UpdMarkingAttendance '" & Session("Company") & "','" _
                       & Session("EmpID") & "','" & EmpID & "','" _
                       & workingdate & "','" & date_time & "','" & errorcode & "','" & leaveCode & "','IN','A','" _
                       & tmsid & "','" & daytypeid & "','" & shiftid & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                myDS = Nothing

                chkCursor.Checked = False
            End If
            chkCursor = myGridView1.Rows(i).Cells(7).Controls(1)
            If chkCursor.Checked = True Then
                workingdate = Trim(myGridview2.Rows(i).Cells(2).Text)
                EmpID = Trim(myGridview2.Rows(i).Cells(3).Text)
                date_time = Trim(myGridview2.Rows(i).Cells(9).Text).Replace("&nbsp;", "")
                errorcode = Trim(myGridview2.Rows(i).Cells(10).Text).Replace("&nbsp;", "")
                leaveCode = Trim(myGridview2.Rows(i).Cells(12).Text).Replace("&nbsp;", "")
                tmsid = Trim(myGridview2.Rows(i).Cells(13).Text).Replace("&nbsp;", "")
                daytypeid = Trim(myGridview2.Rows(i).Cells(14).Text).Replace("&nbsp;", "")
                shiftid = Trim(myGridview2.Rows(i).Cells(15).Text).Replace("&nbsp;", "")
                ...
                ssql = "Exec sp_tms_UpdMarkingAttendance '" & Session("Company") & "','" _
                       & Session("EmpID") & "','" & EmpID & "','" _
                       & workingdate & "','" & date_time & "','" & errorcode & "','" & leaveCode & "','OUT','A','" _
                       & tmsid & "','" & daytypeid & "','" & shiftid & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                myDS = Nothing
    ...
    Private Sub UpdPendingLeaveToReject()
        ...
        For i = 0 To myGridView1.Rows.Count - 1
            ...
            If chkCursor.Checked = True Then
                ...
                ssql = "Exec sp_tms_UpdMarkingAttendance '" & Session("Company") & "','" _
                       & Session("EmpID") & "','" & EmpID & "','" _
                       & workingdate & "','" & date_time & "','" & errorcode & "','" & leaveCode & "','IN','R','" _
                       & tmsid & "','" & daytypeid & "','" & shiftid & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                myDS = Nothing
                ...
                chkCursor.Checked = False
            End If
            chkCursor = myGridView1.Rows(i).Cells(8).Controls(1)
            If chkCursor.Checked = True Then
                workingdate = Trim(myGridview2.Rows(i).Cells(2).Text)
                EmpID = Trim(myGridview2.Rows(i).Cells(3).Text)
                date_time = Trim(myGridview2.Rows(i).Cells(9).Text).Replace("&nbsp;", "")
                ...
                ssql = "Exec sp_tms_UpdMarkingAttendance '" & Session("Company") & "','" _
                       & Session("EmpID") & "','" & EmpID & "','" _
                       & workingdate & "','" & date_time & "','" & errorcode & "','" & leaveCode & "','OUT','R','" _
                       & tmsid & "','" & daytypeid & "','" & shiftid & "'"
                myDS = mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                myDS = Nothing
            End If
...
#Region "Sub & Function 2"
    ...
    Public Sub OptionCheckedChange3()
        If chkAOAll.Checked = True Then
            CheckOAll_Approve()
        Else
            UnCheckOAll_Approve()
        End If
        If chkROAll.Checked = True Then
            CheckOAll_Reject()
        Else
            UnCheckOAll_Reject()
        End If
    ...
    Protected Sub lnkBtnViewAll_Click1(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkBtnViewAll.Click
        lnkBtnViewALL_Click()
    End Sub
    Private Sub lnkBtnViewYesterday_Click()
        CurrentDate = Session("CurrentDate")
        yesterday = mySQL.fn_DateAdd("day", "-1", CurrentDate)
        twodaybefore = mySQL.fn_DateAdd("day", "-2", CurrentDate)
        twoweekbefore = mySQL.fn_DateAdd("day", "-14", CurrentDate)
        ...
        lnkBtnViewYesterday.Visible = False
        lnkBtnCloseYesterday.Visible = True
        pnlpart2.Visible = True

        lnkBtnViewAll.Visible = True
        lnkBtnCloseAll.Visible = False
        lnkBtnViewToday.Visible = True
        lnkBtnCloseToday.Visible = False
        lnkBtnViewApproved.Visible = True
        lnkBtnCloseApproved.Visible = False
        pnlpart3.Visible = False

        Session("currentpage1") = "1"
        Session("TabStatus") = "YES"
    ...
    End Sub
    Private Sub lnkBtnCloseYesterday_Click()
        lnkBtnViewYesterday.Visible = True
        lnkBtnCloseYesterday.Visible = False
        pnlpart2.Visible = False
        pnlpart1.Visible = False
        pnlbutton.Visible = False
    End Sub

    Private Sub lnkBtnViewToday_Click()
        CurrentDate = Session("CurrentDate")
        yesterday = mySQL.fn_DateAdd("day", "-1", CurrentDate)
        twodaybefore = mySQL.fn_DateAdd("day", "-2", CurrentDate)
        twoweekbefore = mySQL.fn_DateAdd("day", "-14", CurrentDate)
        ...
        lnkBtnViewToday.Visible = False
        lnkBtnCloseToday.Visible = True
        pnlpart2.Visible = True
    ...
    Private Sub lnkBtnCloseToday_Click()
        lnkBtnViewToday.Visible = True
        lnkBtnCloseToday.Visible = False
        pnlpart2.Visible = False
        pnlpart1.Visible = False
    ...
    Private Sub lnkBtnViewALL_Click()
        CurrentDate = Session("CurrentDate")
        yesterday = mySQL.fn_DateAdd("day", "-1", CurrentDate)
        twodaybefore = mySQL.fn_DateAdd("day", "-2", CurrentDate)
        twoweekbefore = mySQL.fn_DateAdd("day", "-14", CurrentDate)
        ...
        lstLeaveTotal.Items.Clear()
    End Sub
    Private Sub lnkBtnCloseAll_Click()
        lnkBtnViewAll.Visible = True
        lnkBtnCloseAll.Visible = False
        pnlpart2.Visible = False
        pnlpart1.Visible = False
    ...
    Private Function CheckForAGv1() As Integer
        ...
        Try
            For Each grdItem In myGridView1.Rows
                ...
                chkSelected3 = grdItem.FindControl("chkA3")
                If chkSelected.Checked = True Or chkSelected3.Checked = True Then
                    v_num += 1
                End If
            Next
            Return v_num
    ...
    Private Function CheckForRGv1() As Integer
        ...
        Try
            For Each grdItem In myGridView1.Rows
                ...
                chkSelected3 = grdItem.FindControl("chkR3")
                If chkSelected.Checked = True Or chkSelected3.Checked = True Then
                    v_num += 1
                End If
            Next
    ...
    Private Function CheckForARGv1() As Integer
        ...
        Try
            For Each grdItem In myGridView1.Rows
                ...
                chkSelected3 = grdItem.FindControl("chkA3")
                chkSelected4 = grdItem.FindControl("chkR3")
                If (chkSelected1.Checked = True And chkSelected2.Checked = True) Or (chkSelected3.Checked = True And chkSelected4.Checked = True) Then
                    v_num += 1
                End If
            Next
            Return v_num
    ...
    Private Sub CheckOAll_Approve()
        
        If chkAOAll.Checked = True Then
            For j = 0 To myGridView1.Rows.Count - 1
                Dim chkCursor2 As CheckBox = myGridView1.Rows(j).Cells(7).Controls(1)
                If myGridView1.Rows(j).Cells(9).Text.Replace("&nbsp;", "") <> "" Then
                    chkCursor2.Checked = True
                End If
                chkCursor2 = Nothing
            Next
        End If
    ...
    Private Sub UnCheckOAll_Approve()
        If chkAOAll.Checked = False Then
            For j = 0 To myGridView1.Rows.Count - 1
                Dim chkCursor2 As CheckBox = myGridView1.Rows(j).Cells(7).Controls(1)
                chkCursor2.Checked = False
                chkCursor2 = Nothing
            Next
        End If
    End Sub

    Private Sub CheckOAll_Reject()

        If chkROAll.Checked = True Then
            For j = 0 To myGridView1.Rows.Count - 1
                Dim chkCursor2 As CheckBox = myGridView1.Rows(j).Cells(8).Controls(1)
                If myGridView1.Rows(j).Cells(9).Text.Replace("&nbsp;", "") <> "" Then
                    chkCursor2.Checked = True
                End If
                chkCursor2 = Nothing
    ...
    Private Sub UnCheckOAll_Reject()
        If chkROAll.Checked = False Then
            For j = 0 To myGridView1.Rows.Count - 1
                Dim chkCursor2 As CheckBox = myGridView1.Rows(j).Cells(8).Controls(1)
                chkCursor2.Checked = False
                chkCursor2 = Nothing
    ...
    Private Sub Enable_Approve()
        For i = 0 To myGridView1.Rows.Count - 1
            ...
            chkCursor = myGridView1.Rows(i).Cells(7).Controls(1)
            If myGridView1.Rows(i).Cells(9).Text.Replace("&nbsp;", "") <> "" Then
                chkCursor.Enabled = True
            Else
...
#End Region
    ...
    Protected Sub txtADateFrom_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtADateFrom.TextChanged
        If txtADateFrom.Text > txtADateTo.Text Then
            txtADateTo.Text = txtADateFrom.Text
        End If

    End Sub

    Protected Sub lnkBtnViewToday_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkBtnViewToday.Click
        lnkBtnViewToday_Click()
    End Sub

    Protected Sub ddlSelectAID_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlSelectAID.SelectedIndexChanged
        txtAID.Text = ddlSelectAID.SelectedValue
    End Sub
End Class

...
Partial Class Pages_Roster_RawData_ViewUpdate
...
#End Region
    ...
    Private Sub PagePreload()
        ...
        mySetting.GetDropdownlistValue(Form.ID, "Option_Reason_Type", ddlOption_Reason_Type)
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        If myDS.Tables.Count > 0 Then
            If myDS.Tables(0).Rows.Count > 0 Then
                ssql1 = "Exec sp_tms_InsUpdDelRawData '" & Session("Company").ToString & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','','','','','VIEW'"
                myDS1 = New DataSet()
    ...
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        ElseIf DateTime.TryParseExact(mySetting.UnDisplayDateTime(txtDateOut.Text, Session("Company").ToString, Session("Module").ToString), "yyyyMMddHHmmss", Nothing, DateTimeStyles.None, Nothing) = False And txtDateOut.Text <> "" Then
            ...
            MsgBox(lblresult2.Text)
            txtDateOut.Focus()
        ElseIf txtDateIn.Text = "" And txtDateOut.Text = "" Then
            lblresult2.Text = "Cannot Blank On [" & lblDateIn.Text & "] or [" & lblDateOut.Text & "] !"
            MsgBox(lblresult2.Text)
            txtDateOut.Focus()
        ElseIf txtDateIn.Text <> "" And mySQL.fn_DateDiff("hour", mySetting.UnDisplayDateTime(txtDateIn.Text, Session("Company").ToString, Session("Module").ToString), CurrentDateTime) < 0 Then
            lblresult2.Text = "Cannot Key In [" & lblDateIn.Text & "] for Future !"
            MsgBox(lblresult2.Text)
            txtDateIn.Focus()
        ElseIf txtDateOut.Text <> "" And mySQL.fn_DateDiff("hour", mySetting.UnDisplayDateTime(txtDateOut.Text, Session("Company").ToString, Session("Module").ToString), CurrentDateTime) < 0 Then
            lblresult2.Text = "Cannot Key In [" & lblDateOut.Text & "] for Future !"
            MsgBox(lblresult2.Text)
            txtDateOut.Focus()
        ...
        ElseIf ddlOption_Reason_Type.SelectedValue = "OTHERS" And txtReason.Text = "" Then
            lblresult2.Text = "Cannot Blank On [" & lblReason.Text & "] !"
            MsgBox(lblresult2.Text)
            txtReason.Focus()
        Else
            ssql = "select dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_UnDisplayDate('" & txtDATEFROM.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),dbo.fn_UnDisplayDate('" & txtDATETO.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),isnull(dbo.fn_UnDisplayDatetime('" & txtDateIn.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),''),isnull(dbo.fn_UnDisplayDateTime('" & txtDateOut.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),''),@@spid"
            myDS = New DataSet()
            myDS = mySQL.ExecuteSQL(ssql)
            If myDS.Tables.Count > 0 Then
                If myDS.Tables(0).Rows.Count > 0 Then
                    ...
                    If Trim(myDS2.Tables(0).Rows(0).Item(0).ToString) = "" Then
                        ssql1 = "Exec sp_tms_InsUpdDelRawData '" & Session("Company").ToString & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','" & myDS.Tables(0).Rows(0).Item(4).ToString & "','" & ddlOption_Reason_Type.SelectedValue & "','" & txtReason.Text & "','UPD'"
                        ...
                        If myDS1.Tables.Count > 0 Then
                            If myDS.Tables(0).Rows.Count > 0 Then
                                If myDS1.Tables(0).Rows(0).Item(0).ToString = "1" Then
                                    MsgBox("Duplicate Records.The following Date In already Exists! (" & txtDateIn.Text & ") ")
                                End If
                                If myDS1.Tables(0).Rows(0).Item(1).ToString = "1" Then
                                    MsgBox("Duplicate Records.The following Date Out already Exists! (" & txtDateOut.Text & ") ")
                                End If
                            End If
                        End If
                        txtDateIn.Text = ""
    ...
    Protected Sub ImgBtnDelete_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgBtnDelete.Click
        ...
        If ActionValidateDelete() Then
            For i = 0 To gv1.Rows.Count - 1
                ...
                If chkDelete.Checked And gv1.Rows(i).Cells(4).Text.Replace("&nbsp;", "&").Replace("&amp;", "&").ToUpper = "BY HR" Then
                    If gv2.Rows(i).Cells(2).Text = "IN" Then
                        ssql = "select dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_UnDisplayDate('" & txtDATEFROM.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),dbo.fn_UnDisplayDate('" & txtDATETO.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),'" & gv2.Rows(i).Cells(1).Text & "','',@@spid"
                    Else
                        ssql = "select dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text & "'),dbo.fn_UnDisplayDate('" & txtDATEFROM.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),dbo.fn_UnDisplayDate('" & txtDATETO.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),'','" & gv2.Rows(i).Cells(1).Text & "',@@spid"
                    End If
                    myDS = New DataSet()
                    ...
                    If myDS.Tables.Count > 0 Then
                        If myDS.Tables(0).Rows.Count > 0 Then
                            ...
                            If Trim(myDS2.Tables(0).Rows(0).Item(0).ToString) = "" Then
                                ssql1 = "Exec sp_tms_InsUpdDelRawData '" & Session("Company").ToString & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & myDS.Tables(0).Rows(0).Item(3).ToString & "','" & myDS.Tables(0).Rows(0).Item(4).ToString & "','','','DEL'"

...
Partial Class Pages_Roster_AttendancePaid_View
...
#End Region
    ...
    Protected Sub imgBtnPrint_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnPrint.Click
        ...
        If myDS.Tables.Count > 0 Then
            If myDS.Tables(0).Rows.Count > 0 Then
                ssql1 = "Exec [sp_tms_AttendanceMonthView] '" & Session("Company").ToString & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "'"
                myDS1 = New DataSet()
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        If myds.Tables(0).Rows(0).Item(0).ToString = "" Then
            ...
            Return False
        ElseIf IsNumeric(mySetting.ConvertDateToDecimal(txtDATEFROM.Text, Session("Company").ToString, Session("Module").ToString)) = False Then
            lblresult2.Text = "Invalid Input Date Format For [" & lblDATEFROM.Text & "] !"
            txtDATEFROM.Focus()
            Return False
        ElseIf IsNumeric(mySetting.ConvertDateToDecimal(txtDATETO.Text, Session("Company").ToString, Session("Module").ToString)) = False Then

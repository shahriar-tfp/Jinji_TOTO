...
Partial Class PAGES_INFORMATION_SYSTEM_IMPORT_YEARLY_INCREMENT_UNISEM
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                ...
                '
                'For a = 1 To dtSheet.Rows.Count - 10
                '    sheetname = dtSheet.Rows(a - 1)("TABLE_NAME").ToString.Replace("'", "")
                '    If a = 1 Then
                '        command = "SELECT * FROM [" & sheetname & "a5:z]"
                '        'myData = New OleDbDataAdapter("SELECT * FROM [" & sheetname & "a5:z]", strConn)
                '    Else
                '        command = command + " Union ALL SELECT * FROM [" & sheetname & "a6:z]"
                '        'myData = New OleDbDataAdapter("SELECT * FROM [" & sheetname & "a6:z]", strConn)
                '    End If
                'Next
                'conn.Close()
                'command = "SELECT * FROM [Yearly (10)$a5:z]"
                Dim myData As New OleDbDataAdapter
                myData = New OleDbDataAdapter("SELECT * FROM [Yearly$a5:aa]", strConn)
        ...
        Try
            If GVUpload.Rows.Count > 0 Then
                For i = 0 To GVUpload.Rows.Count - 1
                    ssql1 = "select ID,Company_Profile_Code from employee_profile where code = N'" & GVUpload.Rows(i).Cells(4).Text.Trim.ToUpper.Replace("'", "''") & "'"
                        ...
                        If myds1.Tables(0).Rows(0).Item(0).ToString > 0 Then
                            ...
                            If CompanyID = Session("Company").ToString Then
                                If CInt(employee_id) > 0 Then
                                    ssql = "exec sp_pr_insupddelYearIncrement "
                                    ssql = ssql & " N'" & CompanyID & "',N'" & employee_id & "',"
                                    ssql = ssql & "N'" & GVUpload.Rows(i).Cells(27).Text.Trim.ToUpper.Replace("'", "''") & "0101000000" & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(15).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(17).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(19).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(21).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(23).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(24).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                    ssql = ssql & "'" & GVUpload.Rows(i).Cells(26).Text.Trim.ToUpper.Replace("'", "''") & "',"
                                ...
                                End If
                            Else
                                GVUpload.Rows(i).Cells(0).Text = "Invalid Company Employee Upload!."
                            End If

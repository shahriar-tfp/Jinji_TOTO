...
Partial Class Pages_InformationSystem_EmployeeCode_Transfer
...
#End Region
    ...
    Protected Sub imgBtnSubmit_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnSubmit.Click
        ...
        ssql = "select dbo.fn_ReturnEmpIDByCodeName('" & Session("Company").ToString & "','" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "'),dbo.fn_UnDisplayDate('" & txtEFFECTIVE_DATE.Text & "','" & Session("Company").ToString & "','" & Session("Module").ToString & "'),@@spid"
        myDS = New DataSet()
        myDS = mySQL.ExecuteSQL(ssql)
        lblresult2.Text = ""
        If ValidatePrint(myDS) Then
            ssql = "Exec [sp_is_EmployeeCodeTransfer] '" & TxtNEWEMPLOYEECODE.Text & "','" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & Session("EmpID").ToString & "','" & myDS.Tables(0).Rows(0).Item(1).ToString & "','" & ddlOPTION_TYPE.SelectedValue & "'"
            mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
            ShowMessage(ddlOPTION_TYPE.SelectedItem.Text + " Completed")
            txtEFFECTIVE_DATE.Text = ""
            txtEMPLOYEE_PROFILE_ID.Text = ""
            TxtNEWEMPLOYEECODE.Text = ""
            ddlOPTION_TYPE.SelectedIndex = -1
        End If
    ...
    Private Function ValidatePrint(ByVal myds As DataSet) As Boolean
        ...
        If myds.Tables(0).Rows(0).Item(0).ToString = "" Then
            ...
            Return False
        ElseIf IsNumeric(mySetting.ConvertDateToDecimal(txtEFFECTIVE_DATE.Text, Session("Company").ToString, Session("Module").ToString)) = False Then
            lblresult2.Text = "Invalid Input Date Format For [" & lblEFFECTIVE_DATE.Text & "] !"
            ShowMessage(lblresult2.Text)
            txtEFFECTIVE_DATE.Focus()
            Return False
        ElseIf TxtNEWEMPLOYEECODE.Text = "" Then
            lblresult2.Text = "[" & lblNEWEMPLOYEECODE.Text & "] Is A Required Field!"
            ShowMessage(lblresult2.Text)
            TxtNEWEMPLOYEECODE.Focus()
            Return False
        ElseIf ddlOPTION_TYPE.SelectedValue = "" Then
            ...
            Return False
        Else
            ssql4 = "Exec [sp_is_chkduplicateEmploye] '" & TxtNEWEMPLOYEECODE.Text.Replace("'", "''") & "','CHK'"
            myds4 = mySQL.ExecuteSQL(ssql4, Session("Company").ToString, Session("EmpID").ToString)
            If myds4.Tables.Count > 0 Then
                If myds4.Tables(0).Rows.Count > 0 Then
                    If myds4.Tables(0).Rows(0).Item(0).ToString = "ERROR" Then
                        lblresult2.Text = "[" & TxtNEWEMPLOYEECODE.Text & "] is in use, Please use anothor Code."
                        ShowMessage(lblresult2.Text)
                        TxtNEWEMPLOYEECODE.Focus()
                        Return False
                    End If

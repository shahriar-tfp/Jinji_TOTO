...
Partial Class PAGES_TRANSPORT_EST_EMPROADTAX
...
#Region "Sub & Function"
    ...
    Private Function ValidateUpdate() As Boolean
        ...
        If myDS.Tables(0).Rows.Count > 0 Then
            ...
            myDS2 = Nothing
        End If
        Dim licenceError As String
        licenceError = checkLicence()
        If licenceError.ToString <> "True" Then
            lblresult2.Text = "Invalid Input! Road Tax For " & licenceError.ToString & " Only!"
            Return False
        End If
        Return True
    End Function
    Private Function ReturnSessionValue(ByVal strRequest As String) As String
        ...
        Return strOutput
    End Function
    Private Function checkLicence() As String
        ...
        Dim ssqlf As String, myDSf As DataSet
        ssqlf = "exec sp_EST_SelRoadTaxType '" & empid & "'"
        myDSf = mySQL.ExecuteSQL(ssqlf)

        If myDSf.Tables(0).Rows(0).Item(0).ToString = "1" And myDSf.Tables(0).Rows(0).Item(1).ToString = ddlVEHICLECATEGORY.SelectedValue Then
            Return "True"
        Else
            If myDSf.Tables(0).Rows(0).Item(1).ToString = "C" Then
                Return "Car"
            Else
                Return "Motor"
            End If
...
#End Region
    ...
    Protected Sub txtEMPLOYEE_PROFILE_ID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtEMPLOYEE_PROFILE_ID.TextChanged
        Dim empid As String = mySetting.GetEmployeeIDbyCodeName_ReturnString(Session("Company").ToString, txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''"))
        ssql = "exec sp_EST_SelRoadTaxType '" & empid & "'"
        myDS = mySQL.ExecuteSQL(ssql)

        If imgBtnSearch.Visible = False Then
            If myDS.Tables(0).Rows(0).Item(0).ToString = "1" Then
                ddlVEHICLECATEGORY.SelectedValue = myDS.Tables(0).Rows(0).Item(1).ToString
                ddlVEHICLECATEGORY.Enabled = False
            Else
                ddlVEHICLECATEGORY.SelectedIndex = -1
                ddlVEHICLECATEGORY.Enabled = False
            End If
        End If
    End Sub

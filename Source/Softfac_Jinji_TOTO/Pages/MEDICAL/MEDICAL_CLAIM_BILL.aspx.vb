...
#End Region
    ...
    Protected Sub txtEMPLOYEE_PROFILE_ID_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtEMPLOYEE_PROFILE_ID.TextChanged
        Dim abc As String
        mySetting.GetLookupValue_ImageButton(imgbtnCLAIMTYPE, Form.ID, "txtCLAIMTYPE", "CodeName", "Exec sp_sa_GetLookupCodeName " & """" & Session("Company").ToString & """," & """" & Session("Module").ToString & """," & """" & Form.ID & """," & """" & "CLAIMTYPE" & """," & """" & """," & """" & Session("EmpID").ToString & """", txtEMPLOYEE_PROFILE_ID.Text.Replace("'", """"))
    End Sub
    ...
    Protected Sub txtCLAIMTYPE_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCLAIMTYPE.TextChanged
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "'),dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "'), dbo.fn_GetCode_MEDICAL_CLAIMTYPE_Vw(N'" & txtCLAIMTYPE.Text & "')"
        myDS = mySQL.ExecuteSQL(ssql)
    ...
    Protected Sub imgBtnUpdate_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpdate.Click
        ssql = "select dbo.fn_ReturnCompanyProfileCode(N'" & txtCOMPANY_PROFILE_CODE.Text & "')," & _
        "dbo.fn_ReturnEmpIDByCodeName(N'" & Session("Company").ToString & "',N'" & txtEMPLOYEE_PROFILE_ID.Text.Replace("'", "''") & "'), " & _
            ...
            If myDS.Tables(0).Rows.Count = 1 Then
                If txtAUTOAPPROVALNO.Text = "" Then
                    ssql1 = "exec sp_md_InsUpdDelMedicalTransaction '" & myDS.Tables(0).Rows(0).Item(0).ToString & "','" & _
                            myDS.Tables(0).Rows(0).Item(1).ToString & "','" & myDS.Tables(0).Rows(0).Item(2).ToString & "','" & _
                            myDS.Tables(0).Rows(0).Item(3).ToString & "','" & ddlBENEFICIARYVISITER.SelectedValue.Replace("'", "''") & "','" & _
                            mySetting.ConvertDateToDecimal(Trim(txtVISITDATE.Text), Session("Company").ToString, Session("Module").ToString) & "','" & _
    ...
    Private Sub ClearText()
        ...
        txtCLAIMTYPE.Text = ""
        ddlBENEFICIARYVISITER.SelectedIndex = -1
        txtVISITDATE.Text = ""
        txtENTITLEMENTAMT.Enabled = True
        txtENTITLEMENTAMT.Text = ""
        txtUPTODATE_BALANCE.Enabled = True
        txtUPTODATE_BALANCE.Text = ""
        txtCLAIMAMT.Enabled = True
        txtCLAIMAMT.Text = ""
        txtBILLAMT.Text = ""
        'myDS = New DataSet
    ...
    Protected Sub txtVISITDATE_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtVISITDATE.TextChanged
        txtBILLDATE.Text = txtVISITDATE.Text
        txtPAYMENTMONTH.Text = mySetting.GetCurrentDate(Session("Company").ToString, Session("Module").ToString)
    End Sub

    Protected Sub imgbtnCLAIMTYPE_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgbtnCLAIMTYPE.Click
        txtENTITLEMENTAMT.Enabled = True
        txtUPTODATE_BALANCE.Enabled = True
        txtCLAIMAMT.Enabled = True
    End Sub

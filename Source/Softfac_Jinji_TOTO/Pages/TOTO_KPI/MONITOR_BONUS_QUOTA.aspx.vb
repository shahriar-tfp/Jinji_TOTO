...
Partial Class Pages_TOTO_KPI_MONITOR_BONUS_QUOTA
...
#End Region
    ...
    Protected Sub blindDepartment()
        ssql1 = "Exec sp_KPI_SelRptDropDownlist '" & Session("Company").ToString & "','" & Session("EmpID").ToString & "','" & ddlCATEGORY.SelectedValue & "','','','','','','','','BLINDDept','" & ddlYEAR.SelectedValue & "'"
        myDS1 = New DataSet()
    ...
    Protected Sub gv1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType <> DataControlRowType.Header Then
            Dim sa As String
            sa = e.Row.Cells(0).Text
            If ddlCATEGORY.SelectedValue = "INDIRECT" And e.Row.Cells(0).Text = "S" Then
                e.Row.Cells(1).RowSpan = 2
                e.Row.Cells(3).RowSpan = 2
            ElseIf ddlCATEGORY.SelectedValue = "INDIRECT" And e.Row.Cells(0).Text = "A" Then
    ...
    Protected Sub blindDirect()
        ...
        If ddlCATEGORY.SelectedValue = "INDIRECT" Then
            ssql1 = "Create table #result(Code nvarchar(50), Name nvarchar(200)); insert into #result select 'Man','Management' union select 'Non','Non-Management'; select Code, Name from #result;Drop table #result;"
        Else
            ssql1 = "Create table #result(Code nvarchar(50), Name nvarchar(200)); insert into #result select 'Non','Non-Management'; select Code, Name from #result;Drop table #result;"
        ...
        If myDS1.Tables.Count > 0 Then
            ddlMANAGEMENT.DataSource = myDS1.Tables(0)
            ddlMANAGEMENT.DataTextField = "Name"
            ddlMANAGEMENT.DataValueField = "Code"
    ...
    Protected Sub blindFinal()
        ...
        ssql1 = "Exec sp_KPI_RptBonusQuota '" & Session("EmpID").ToString & "','" & ddlCATEGORY.SelectedValue & "','" & ddlOCP_ID_DEPARTMENT.SelectedValue & "','" & ddlMANAGEMENT.SelectedValue & "','" & ddlYEAR.SelectedValue & "'"
        myDS1 = New DataSet()
        myDS1 = mySQL.ExecuteSQL(ssql1)
        If myDS1.Tables.Count = 1 Then
            ...
            For i As Integer = 0 To gv1.Rows.Count - 1
                ...
                Dim item1 As String = Server.HtmlDecode(row.Cells(1).Text)
                Dim item2 As String = Server.HtmlDecode(row.Cells(3).Text)
                If item1 <> "No limit" And IsNumeric(item2.Replace("%", "")) = True Then
                    If CDbl(item1.Replace("%", "")) < CDbl(item2.Replace("%", "")) Then

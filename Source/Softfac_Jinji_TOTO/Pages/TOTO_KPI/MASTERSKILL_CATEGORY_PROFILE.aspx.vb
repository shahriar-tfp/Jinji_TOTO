...
Partial Class PAGES_TOTO_KPI_MASTERSKILL_CATEGORY_PROFILE
...
#Region "Sub & Function"
    ...
    Private Sub UncheckAll()
        ...
        Next
    End Sub
    Private Function duplicateRecord() As Boolean
        Dim company As String
        company = mySetting.GetCompanyCodebyCodeName_ReturnString(txtCOMPANY_PROFILE_CODE.Text)

        If company = "" Then
            lblresult2.Text = "Company Code Was Wrong!"
            ShowMessage(lblresult2.Text)
            Return False
        ElseIf txtNAME.Text = "" Then
            lblresult2.Text = "Name Cannot Be Blank!"
            ShowMessage(lblresult2.Text)
            Return False
        Else
            ssql3 = "Exec sp_KPI_ChkDuplicate 'ChkI','" & txtNAME.Text.Replace("'", "''") & "'"
            myDS = mySQL.ExecuteSQL(ssql3, Session("Company").ToString, Session("EmpID").ToString)
            If myDS.Tables.Count > 1 Then
                If myDS.Tables(0).Rows(0).Item(0).ToString = "YES" Then
                    lblresult2.Text = "Duplicate Record Found!"
                    ShowMessage(lblresult2.Text)
                    Return False
                Else

                    ssql = "Exec sp_KPI_InsMaster 'AddI','" & company & "','" & myDS.Tables(1).Rows(0).Item(0).ToString & "','" & txtNAME.Text.Replace("'", "''") & "','" & Session("EmpID").ToString & "'"
                End If
            End If
        End If
        Return True

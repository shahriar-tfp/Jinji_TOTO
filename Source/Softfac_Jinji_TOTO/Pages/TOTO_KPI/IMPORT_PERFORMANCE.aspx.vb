...
Partial Class PAGES_TOTO_KPI_IMPORT_PERFORMANCE
    ...
    Protected Sub imgBtnUpload_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgBtnUpload.Click
        If Not (txtFileName.PostedFile Is Nothing) Then
            Try
                ...
                Dim myData As New OleDbDataAdapter("SELECT * FROM [Final Upload$]", strConn)
                myData.TableMappings.Add("Table", "ExcelTest")
                myData.Fill(myDS)
                Dim foundrow() As Data.DataRow
                foundrow = myDS.Tables(0).Select("EmpCode = ''")
                Dim i As Integer

                For i = 0 To foundrow.Length - 1
                    myDS.Tables(0).Rows.Remove(foundrow(i))
                Next
        ...
        Try
            If GVUpload.Rows.Count > 0 Then
                For i = 0 To GVUpload.Rows.Count - 1
                    ...
                    ssql = "insert into IMPORT_PERFORMANCE_TOTO_KPI "
                    ssql = ssql & " Select N'" & GVUpload.Rows(i).Cells(1).Text.Trim.ToUpper.Replace("'", "''") & "',N'" & GVUpload.Rows(i).Cells(2).Text.Trim.ToUpper.Replace("'", "''") & "',"
                    ssql = ssql & "N'" & GVUpload.Rows(i).Cells(3).Text.Trim.ToUpper.Replace("'", "''") & "','" & ddlYear.SelectedValue & "','" & ddlMonth.SelectedValue & "',"
                    ssql = ssql & "N'" & Trim(GVUpload.Rows(i).Cells(4).Text.Trim.ToUpper.Replace("'", "''")) & "',N'" & Trim(GVUpload.Rows(i).Cells(5).Text.Trim.ToUpper.Replace("'", "''")) & "',"
                    ssql = ssql & "N'" & Trim(GVUpload.Rows(i).Cells(6).Text.Trim.ToUpper.Replace("'", "''")) & "',N'" & Trim(GVUpload.Rows(i).Cells(7).Text.Trim.ToUpper.Replace("'", "''")) & "',"
                    ssql = ssql & "N'" & Trim(GVUpload.Rows(i).Cells(8).Text.Trim.ToUpper.Replace("'", "''")) & "',N'" & Trim(GVUpload.Rows(i).Cells(9).Text.Trim.ToUpper.Replace("'", "''")) & "',"
                    ssql = ssql & "N'" & Trim(GVUpload.Rows(i).Cells(10).Text.Trim.ToUpper.Replace("'", "''")) & "',N'" & Trim(GVUpload.Rows(i).Cells(11).Text.Trim.ToUpper.Replace("'", "''")) & "',"
                    ssql = ssql & "''"
                    mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                    ssql = Nothing
                ...
                Next
                ssql = "Exec sp_Kpi_InsUpdDelPerformance '" & Session("EmpID").ToString & "'"
                mySQL.ExecuteSQL(ssql, Session("Company").ToString, Session("EmpID").ToString)
                ssql = Nothing
            End If
            'GVUpload.DataBind()
